...
def predict():
    ...
    model_ready = pipeline_available()
    ...
    if request.method == "POST" and model_ready:
        try:
            result = predict_rain_tomorrow(dict(request.form))
    ...
    wind_series = pd.concat(
        [df["WindGustDir"], df["WindDir9am"], df["WindDir3pm"]],
        ignore_index=True,
    ).dropna()
    wind_dirs = sorted(wind_series.astype(str).unique().tolist())
